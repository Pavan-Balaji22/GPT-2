...
def esitimate_loss():
    ...
    for mode in ["train", "val"]:
        ...
        for i in range(hconfig.eval_iter):
            X, Y = DataLoader.get_batch(mode, hconfig)
            _, lossb = model(X, Y)
            losses = losses + lossb
...
class GPT(nn.Module):
    ...
    def generate(self, max_tokens: int, idx: torch.Tensor, config):
        ...
        for _ in range(max_tokens):
            ...
            logits = logits[
                :, -1, :
            ]  # B,C Picking last time step as it is the predictoin of next letter
